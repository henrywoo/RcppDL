...
// SdA
SdA::SdA(int size, int n_i, vector<int> hls, int n_o, int n_l) {
	int input_size;
	...
	for (int i = 0; i < n_layers; i++) {
		...
		sigmoid_layers[i] = new HiddenLayer(N, input_size,
	...
	log_layer = new LogisticRegression(N, hidden_layer_sizes[n_layers - 1],
			n_outs);
}
			...
			for (int n = 0; n < N; n++)   // input x1...xN
				...
				for (int l = 0; l <= i; l++) {
					...
					if (l == 0) {
						...
						sigmoid_layers[l - 1]->sample_h_given_v(
	...
	for (int epoch = 0; epoch < epochs; epoch++) {
		for (int n = 0; n < N; n++)   // input x1...xN
			...
			for (int i = 0; i < n_layers; i++) {
				...
				sigmoid_layers[i]->sample_h_given_v(prev_layer_input,
						layer_input);
				delete[] prev_layer_input;
			}
...
void SdA::predict(int *x, vector<double>& y) {
	double *layer_input;
	int prev_layer_input_size;
	...
	for (int i = 0; i < n_layers; i++) {
		...
		for (int k = 0; k < sigmoid_layers[i]->n_out; k++) {
			...
			for (int j = 0; j < sigmoid_layers[i]->n_in; j++) {
				linear_output += sigmoid_layers[i]->W[k][j]
						* prev_layer_input[j];
			}
